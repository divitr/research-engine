...
def init():
    ...
    model = EncoderDecoderModel.from_pretrained(
        'mrm8488/bert-small2bert-small-finetuned-cnn_daily_mail-summarization').to(device)
    kw_model = KeyBERT()
...
def extract(url):
    ...
    data = {"url": url, "title": title.renderContents().decode("utf-8"),
            "text": text}
    return data
...
def generate_summary(text):
    ...
    output = model.generate(
        input_ids, attention_mask=attention_mask, max_new_tokens=200)
...
def keywords(text):
    keywords = kw_model.extract_keywords(text, keyphrase_ngram_range=(
        1, 2), stop_words='english', top_n=3, use_mmr=True)
...
def getURLS(query):
    return app.response_class(response=json.dumps(get_search_results(query, 5)), mimetype='application/json')
...
testinfo = [
    ...
    {
        "url": "https://www.medicalnewstoday.com/articles/161618"
    },
    {
        "url": "https://my.clevelandclinic.org/health/diseases/15050-vitamin-d-vitamin-d-deficiency"
    },
...
@app.route('/testinfo')
def testInfo():
    return app.response_class(response=json.dumps(testinfo))


@app.route('/getPageText/')
def getPageText():
  url = request.args.get('url')
  print("\n\n" + url)
  print("DECODED" + urllib.parse.unquote(url) + "\n\n")
  time.sleep(random.randrange(1,5))
  return app.response_class(response=json.dumps(extract(url)), mimetype='application/json')
...
@app.route('/testsummary', methods=['GET', 'POST'])
def summ():
    time.sleep(3)
